      ...
      <div className="h-full max-h-[calc(100vh-215px)] overflow-y-auto p-2 space-y-2 custom-scroll">
        {!selecteduser ? (
          <p className="text-gray-400 text-center mt-20">
            Pls select a user to start a convo...
          </p>
        ) : (
          <>
            {uniqueMessages.map((e, i) => {
              ...
              return (
                <div key={uniqueKey} className={`flex flex-col gap-1 mt-2 ${isSentByMe ? "items-end" : "items-start"}`}>
                  ...
                  <div className={`flex flex-col items-center gap-1 text-xs text-gray-400 ${isSentByMe ? "pr-2 flex-row-reverse" : "pl-2"}`}>
                  {/* <div className={`flex flex-col items-center gap-1 text-xs text-gray-400`}> */}
                    <span>{moment(e.createdAt).format("hh:mm A . MMM DD")}</span>
                    {isSentByMe && (
                      <div className="flex items-center ml-1">
                        {e.seen ? (
                          <div className="flex items-center gap-1 text-blue-400">
                            ...
                            {e.seenAt && (
                              <span>{moment(e.seenAt).format("hh:mm A . MMM DD")}</span>
                            )}
                          </div>
                        ) : (
