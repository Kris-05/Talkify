...
const VerifyPage = () => {
  ...
  // to get email from "/verify?email=${user mail}"
  ...
  // delete
  const handleKeyDown = (idx: number, e: React.KeyboardEvent<HTMLElement>): void => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { data } = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v2/verify`, {
        email,
        otp: otpString,
      });
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const { data } = await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v2/login`, {
        email
      });
      ...
      <div className='max-w-md w-full'>
        <div className='bg-gray-800 border-gray-700 rounded-lg p-8'>
          ...
          <div className='mt-6 text-center'>
            ...
            {timer > 0 ? (
              <p className='text-gray-300 text-sm mb-4'>Resend available in {timer}s</p>
            ) : (
              <button 
